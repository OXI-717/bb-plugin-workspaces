...
function createDeps(initialProjects: ProjectRecord[], storedProjectId: string | null = null) {
  ...
  const calls: { create: CreateProjectInput[]; addSource: Array<{ projectId: string; input: LocalPathSourceInput }> } = {
    create: [], addSource: [],
  };
...
describe("ensureWorkspaceProject", () => {
  ...
  it("attaches the anchor source when the stored project is absent from the host", async () => {
    const existing = project("proj_workspaces", "🧩 Workspaces", [source("host_remote", "/plugin/anchor")]);
  ...
  it("does not adopt a user project that only collides on display name", async () => {
    const userProject = project("proj_user", "🧩 Workspaces", [source("host_local", "/repos/user-workspaces")]);
