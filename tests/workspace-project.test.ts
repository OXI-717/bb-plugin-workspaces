...
describe("ensureWorkspaceProject", () => {
  ...
  it.each(["🧩 Workspaces", "🗂️ Workspace Hub"])("renames the stored legacy synthetic project %s without replacing it", async (legacyName) => {
    const legacy = project("proj_workspaces", legacyName, [source("host_local", "/plugin/anchor")]);
    const { deps, calls, getProjects } = createDeps([legacy], "proj_workspaces");

    expect(await ensureWorkspaceProject(deps, "host_local")).toBe("proj_workspaces");
    expect(calls.create).toEqual([]);
    expect(calls.updateName).toEqual([{ projectId: "proj_workspaces", name: "Workspace Hub" }]);
    expect(getProjects()[0]?.name).toBe("Workspace Hub");
  });
  ...
  it("attaches the anchor source when the stored project is absent from the host", async () => {
    const existing = project("proj_workspaces", "Workspace Hub", [source("host_remote", "/plugin/anchor")]);
  ...
  it("does not adopt a user project that only collides on display name", async () => {
    const userProject = project("proj_user", "Workspace Hub", [source("host_local", "/repos/user-workspaces")]);
